// Made with Blockbench 4.1.1
// Exported for Minecraft version 1.15 - 1.16 with MCP mappings
// Paste this class into your mod and generate all required imports


public class Thegreedyfoxgoddess extends EntityModel<Entity> {
	private final ModelRenderer support;
	private final ModelRenderer cube_r1;
	private final ModelRenderer cube_r2;
	private final ModelRenderer cube_r3;
	private final ModelRenderer cube_r4;
	private final ModelRenderer cube_r5;
	private final ModelRenderer cube_r6;
	private final ModelRenderer cube_r7;
	private final ModelRenderer cube_r8;
	private final ModelRenderer cube_r9;
	private final ModelRenderer Main;
	private final ModelRenderer head;
	private final ModelRenderer bone14;
	private final ModelRenderer body;
	private final ModelRenderer bone13;
	private final ModelRenderer cube_r10;
	private final ModelRenderer bone12;
	private final ModelRenderer bone11;
	private final ModelRenderer cube_r11;
	private final ModelRenderer bone10;
	private final ModelRenderer cube_r12;
	private final ModelRenderer cube_r13;
	private final ModelRenderer cube_r14;
	private final ModelRenderer tail;
	private final ModelRenderer bone;
	private final ModelRenderer cube_r15;
	private final ModelRenderer cube_r16;
	private final ModelRenderer cube_r17;
	private final ModelRenderer bone2;
	private final ModelRenderer cube_r18;
	private final ModelRenderer cube_r19;
	private final ModelRenderer cube_r20;
	private final ModelRenderer bone3;
	private final ModelRenderer cube_r21;
	private final ModelRenderer cube_r22;
	private final ModelRenderer cube_r23;
	private final ModelRenderer bone4;
	private final ModelRenderer cube_r24;
	private final ModelRenderer cube_r25;
	private final ModelRenderer cube_r26;
	private final ModelRenderer bone5;
	private final ModelRenderer cube_r27;
	private final ModelRenderer cube_r28;
	private final ModelRenderer cube_r29;
	private final ModelRenderer bone6;
	private final ModelRenderer cube_r30;
	private final ModelRenderer cube_r31;
	private final ModelRenderer cube_r32;
	private final ModelRenderer bone7;
	private final ModelRenderer cube_r33;
	private final ModelRenderer cube_r34;
	private final ModelRenderer cube_r35;
	private final ModelRenderer bone8;
	private final ModelRenderer cube_r36;
	private final ModelRenderer cube_r37;
	private final ModelRenderer cube_r38;
	private final ModelRenderer bone9;
	private final ModelRenderer cube_r39;
	private final ModelRenderer cube_r40;
	private final ModelRenderer cube_r41;

	public Thegreedyfoxgoddess() {
		textureWidth = 200;
		textureHeight = 200;

		support = new ModelRenderer(this);
		support.setRotationPoint(0.0F, 23.0F, 4.0F);
		setRotationAngle(support, 0.3054F, 0.0435F, 0.0038F);
		support.setTextureOffset(87, 16).addBox(-4.4F, -10.7F, -6.5F, 10.0F, 10.0F, 5.0F, 0.0F, false);
		support.setTextureOffset(81, 23).addBox(-14.4F, 2.6F, -11.2F, 10.0F, 6.0F, 14.0F, 0.0F, false);
		support.setTextureOffset(81, 23).addBox(-4.4F, 0.9F, -10.9F, 10.0F, 4.0F, 14.0F, 0.0F, false);
		support.setTextureOffset(81, 23).addBox(5.6F, 0.9F, -10.9F, 10.0F, 6.0F, 14.0F, 0.0F, false);
		support.setTextureOffset(81, 23).addBox(-13.4F, 0.9F, -11.8F, 10.0F, 6.0F, 14.0F, 0.0F, false);
		support.setTextureOffset(83, 23).addBox(-15.4F, -8.4F, -11.5F, 5.0F, 11.0F, 17.0F, 0.0F, false);
		support.setTextureOffset(83, 23).addBox(11.6F, -8.4F, -11.5F, 5.0F, 11.0F, 17.0F, 0.0F, false);

		cube_r1 = new ModelRenderer(this);
		cube_r1.setRotationPoint(0.0F, -0.4F, -1.3F);
		support.addChild(cube_r1);
		setRotationAngle(cube_r1, -0.2182F, 0.0F, 0.0F);
		cube_r1.setTextureOffset(83, 23).addBox(-10.4F, -11.1F, -11.2F, 6.0F, 14.0F, 17.0F, 0.0F, false);
		cube_r1.setTextureOffset(83, 23).addBox(5.6F, -11.1F, -11.2F, 6.0F, 14.0F, 17.0F, 0.0F, false);
		cube_r1.setTextureOffset(84, 16).addBox(-4.4F, -8.5F, -1.2F, 10.0F, 10.0F, 7.0F, 0.0F, false);

		cube_r2 = new ModelRenderer(this);
		cube_r2.setRotationPoint(0.0F, 0.0F, 0.0F);
		support.addChild(cube_r2);
		setRotationAngle(cube_r2, 0.0F, 0.2182F, 0.0F);
		cube_r2.setTextureOffset(83, 23).addBox(-3.5F, -2.2F, -6.7F, 6.0F, 12.0F, 9.0F, 0.0F, false);
		cube_r2.setTextureOffset(83, 23).addBox(-3.2F, -2.0F, -5.7F, 5.0F, 14.0F, 7.0F, 0.0F, false);
		cube_r2.setTextureOffset(83, 23).addBox(-2.2F, 3.2F, -4.7F, 3.0F, 14.0F, 5.0F, 0.0F, false);

		cube_r3 = new ModelRenderer(this);
		cube_r3.setRotationPoint(0.0F, 0.0F, 0.0F);
		support.addChild(cube_r3);
		setRotationAngle(cube_r3, 0.0F, 0.1745F, 0.0F);
		cube_r3.setTextureOffset(83, 23).addBox(0.9F, -3.7F, -7.8F, 5.0F, 14.0F, 7.0F, 0.0F, false);
		cube_r3.setTextureOffset(83, 23).addBox(2.0F, 1.6F, -6.8F, 3.0F, 14.0F, 5.0F, 0.0F, false);

		cube_r4 = new ModelRenderer(this);
		cube_r4.setRotationPoint(0.0F, 0.0F, 0.0F);
		support.addChild(cube_r4);
		setRotationAngle(cube_r4, 0.0F, 0.1309F, 0.0F);
		cube_r4.setTextureOffset(83, 23).addBox(9.1F, -3.5F, -7.5F, 5.0F, 14.0F, 7.0F, 0.0F, false);
		cube_r4.setTextureOffset(83, 23).addBox(10.1F, 2.5F, -6.5F, 3.0F, 14.0F, 5.0F, 0.0F, false);

		cube_r5 = new ModelRenderer(this);
		cube_r5.setRotationPoint(0.0F, 0.0F, 0.0F);
		support.addChild(cube_r5);
		setRotationAngle(cube_r5, 0.0F, -0.0873F, 0.0F);
		cube_r5.setTextureOffset(83, 23).addBox(6.1F, 0.2F, -7.5F, 5.0F, 14.0F, 7.0F, 0.0F, false);
		cube_r5.setTextureOffset(83, 23).addBox(7.1F, 6.2F, -6.5F, 3.0F, 14.0F, 5.0F, 0.0F, false);
		cube_r5.setTextureOffset(83, 23).addBox(5.6F, -1.3F, -8.5F, 6.0F, 13.0F, 9.0F, 0.0F, false);

		cube_r6 = new ModelRenderer(this);
		cube_r6.setRotationPoint(0.0F, 0.0F, 0.0F);
		support.addChild(cube_r6);
		setRotationAngle(cube_r6, 0.0F, 0.2618F, 0.0F);
		cube_r6.setTextureOffset(83, 23).addBox(-9.3F, 11.7F, -8.8F, 3.0F, 14.0F, 5.0F, 0.0F, false);
		cube_r6.setTextureOffset(83, 23).addBox(-10.3F, 5.7F, -9.8F, 5.0F, 14.0F, 7.0F, 0.0F, false);
		cube_r6.setTextureOffset(83, 23).addBox(-11.3F, -1.3F, -10.8F, 7.0F, 15.0F, 9.0F, 0.0F, false);

		cube_r7 = new ModelRenderer(this);
		cube_r7.setRotationPoint(0.0F, -0.4F, -1.3F);
		support.addChild(cube_r7);
		setRotationAngle(cube_r7, -0.4102F, 0.0F, 0.0F);
		cube_r7.setTextureOffset(84, 16).addBox(-7.4F, -18.5F, 1.9F, 17.0F, 5.0F, 4.0F, 0.0F, false);
		cube_r7.setTextureOffset(84, 16).addBox(-7.4F, -23.5F, 1.9F, 17.0F, 5.0F, 4.0F, 0.0F, false);

		cube_r8 = new ModelRenderer(this);
		cube_r8.setRotationPoint(0.0F, -0.4F, -1.3F);
		support.addChild(cube_r8);
		setRotationAngle(cube_r8, -0.3229F, 0.0F, 0.0F);
		cube_r8.setTextureOffset(81, 16).addBox(-9.4F, -13.5F, 0.9F, 10.0F, 5.0F, 5.0F, 0.0F, false);
		cube_r8.setTextureOffset(81, 16).addBox(0.6F, -13.5F, 0.9F, 10.0F, 5.0F, 5.0F, 0.0F, false);

		cube_r9 = new ModelRenderer(this);
		cube_r9.setRotationPoint(0.0F, -0.4F, -1.3F);
		support.addChild(cube_r9);
		setRotationAngle(cube_r9, -0.0873F, 0.0F, 0.0F);
		cube_r9.setTextureOffset(87, 16).addBox(-4.4F, -2.8F, -11.5F, 10.0F, 4.0F, 5.0F, 0.0F, false);

		Main = new ModelRenderer(this);
		Main.setRotationPoint(-7.0F, 23.0F, 4.0F);
		setRotationAngle(Main, -0.1581F, -0.2985F, 0.0911F);
		

		head = new ModelRenderer(this);
		head.setRotationPoint(5.6F, -23.1F, -2.5F);
		Main.addChild(head);
		setRotationAngle(head, -0.2182F, 0.0F, 0.0F);
		

		bone14 = new ModelRenderer(this);
		bone14.setRotationPoint(0.1F, -0.7F, 0.0F);
		head.addChild(bone14);
		setRotationAngle(bone14, 0.256F, 0.2969F, 0.0462F);
		bone14.setTextureOffset(42, 7).addBox(-1.9293F, -4.9991F, -4.4539F, 1.0F, 1.0F, 1.0F, 0.0F, false);
		bone14.setTextureOffset(23, 9).addBox(-0.9293F, -4.9991F, -4.4539F, 1.0F, 2.0F, 1.0F, 0.0F, false);
		bone14.setTextureOffset(23, 8).addBox(0.0707F, -4.9991F, -4.4539F, 1.0F, 3.0F, 1.0F, 0.0F, false);
		bone14.setTextureOffset(24, 9).addBox(-3.9293F, -4.9991F, -4.4539F, 1.0F, 4.0F, 1.0F, 0.0F, false);
		bone14.setTextureOffset(23, 9).addBox(3.0707F, -3.9991F, -4.4539F, 1.0F, 4.0F, 1.0F, 0.0F, false);
		bone14.setTextureOffset(43, 9).addBox(-3.9293F, -0.9991F, -4.4539F, 1.0F, 1.0F, 1.0F, 0.0F, false);
		bone14.setTextureOffset(43, 9).addBox(3.0707F, -4.9991F, -4.4539F, 1.0F, 1.0F, 1.0F, 0.0F, false);
		bone14.setTextureOffset(43, 9).addBox(2.0707F, -4.9991F, -4.4539F, 1.0F, 1.0F, 1.0F, 0.0F, false);
		bone14.setTextureOffset(44, 10).addBox(1.0707F, -4.9991F, -4.4539F, 1.0F, 1.0F, 1.0F, 0.0F, false);
		bone14.setTextureOffset(14, 8).addBox(-3.9293F, -7.9991F, -4.4539F, 8.0F, 3.0F, 1.0F, 0.0F, false);
		bone14.setTextureOffset(7, 8).addBox(-3.9293F, -9.9991F, -2.3539F, 2.0F, 2.0F, 1.0F, 0.0F, false);
		bone14.setTextureOffset(8, 8).addBox(2.0707F, -9.9991F, -2.3539F, 2.0F, 2.0F, 1.0F, 0.0F, false);
		bone14.setTextureOffset(8, 7).addBox(2.0707F, -10.9991F, -2.3539F, 2.0F, 1.0F, 1.0F, 0.0F, false);
		bone14.setTextureOffset(12, 7).addBox(-3.9293F, -10.9991F, -2.3539F, 2.0F, 1.0F, 1.0F, 0.0F, false);
		bone14.setTextureOffset(12, 7).addBox(-2.9293F, -11.9991F, -2.3539F, 1.0F, 1.0F, 1.0F, 0.0F, false);
		bone14.setTextureOffset(12, 7).addBox(2.0707F, -11.9991F, -2.3539F, 1.0F, 1.0F, 1.0F, 0.0F, false);
		bone14.setTextureOffset(68, 6).addBox(4.0707F, -5.9991F, -3.4539F, 1.0F, 1.0F, 4.0F, 0.0F, false);
		bone14.setTextureOffset(68, 6).addBox(4.0707F, -5.9991F, 0.5461F, 1.0F, 1.0F, 4.0F, 0.0F, false);
		bone14.setTextureOffset(68, 6).addBox(-4.9293F, -5.9991F, -3.4539F, 1.0F, 1.0F, 4.0F, 0.0F, false);
		bone14.setTextureOffset(68, 6).addBox(-4.9293F, -6.9991F, -4.4539F, 1.0F, 4.0F, 1.0F, 0.0F, false);
		bone14.setTextureOffset(68, 6).addBox(4.0707F, -6.9991F, -4.4539F, 1.0F, 4.0F, 1.0F, 0.0F, false);
		bone14.setTextureOffset(68, 6).addBox(-4.9293F, -5.9991F, 0.5461F, 1.0F, 1.0F, 4.0F, 0.0F, false);
		bone14.setTextureOffset(68, 6).addBox(0.0707F, -5.9991F, 4.5461F, 5.0F, 1.0F, 1.0F, 0.0F, false);
		bone14.setTextureOffset(68, 6).addBox(2.0707F, -5.9991F, -5.4539F, 3.0F, 1.0F, 1.0F, 0.0F, false);
		bone14.setTextureOffset(68, 6).addBox(-4.9293F, -5.9991F, -5.4539F, 3.0F, 1.0F, 1.0F, 0.0F, false);
		bone14.setTextureOffset(68, 6).addBox(-4.9293F, -5.9991F, 4.5461F, 5.0F, 1.0F, 1.0F, 0.0F, false);
		bone14.setTextureOffset(0, 0).addBox(-3.9293F, -7.9991F, -3.4539F, 8.0F, 8.0F, 8.0F, 0.0F, false);
		bone14.setTextureOffset(32, 0).addBox(-3.9293F, -7.9991F, -3.4539F, 8.0F, 8.0F, 8.0F, 0.0F, false);

		body = new ModelRenderer(this);
		body.setRotationPoint(0.0F, 0.0F, 0.0F);
		Main.addChild(body);
		

		bone13 = new ModelRenderer(this);
		bone13.setRotationPoint(3.0F, -2.6F, 2.0F);
		body.addChild(bone13);
		setRotationAngle(bone13, 0.0F, 0.1745F, 0.0F);
		

		cube_r10 = new ModelRenderer(this);
		cube_r10.setRotationPoint(-3.0F, 2.6F, -2.0F);
		bone13.addChild(cube_r10);
		setRotationAngle(cube_r10, -0.2182F, 0.0F, 0.0F);
		cube_r10.setTextureOffset(16, 16).addBox(2.6021F, -23.0F, -9.4612F, 8.0F, 12.0F, 4.0F, 0.0F, false);

		bone12 = new ModelRenderer(this);
		bone12.setRotationPoint(3.2F, -2.1F, 2.0F);
		body.addChild(bone12);
		setRotationAngle(bone12, 0.0872F, 0.3054F, 0.0F);
		bone12.setTextureOffset(23, 20).addBox(0.6021F, -11.6F, -8.8612F, 4.0F, 4.0F, 4.0F, 0.0F, false);
		bone12.setTextureOffset(160, 24).addBox(0.4682F, -10.6372F, -4.9811F, 4.0F, 4.0F, 4.0F, 0.0F, false);
		bone12.setTextureOffset(22, 20).addBox(0.6021F, -11.6F, -12.8612F, 4.0F, 4.0F, 4.0F, 0.0F, false);
		bone12.setTextureOffset(29, 53).addBox(0.6021F, -7.6F, -12.8612F, 4.0F, 2.0F, 4.0F, 0.0F, false);
		bone12.setTextureOffset(16, 56).addBox(0.6021F, -5.6F, -12.8612F, 4.0F, 4.0F, 4.0F, 0.0F, false);

		bone11 = new ModelRenderer(this);
		bone11.setRotationPoint(3.0F, -2.4F, 2.0F);
		body.addChild(bone11);
		setRotationAngle(bone11, 0.2101F, -0.0029F, -0.0092F);
		

		cube_r11 = new ModelRenderer(this);
		cube_r11.setRotationPoint(-3.0F, 2.6F, -2.0F);
		bone11.addChild(cube_r11);
		setRotationAngle(cube_r11, -0.1749F, 0.0869F, -0.0076F);
		cube_r11.setTextureOffset(23, 20).addBox(5.4021F, -13.8F, -8.1612F, 4.0F, 4.0F, 4.0F, 0.0F, false);
		cube_r11.setTextureOffset(16, 56).addBox(5.4021F, -7.8F, -12.1612F, 4.0F, 4.0F, 4.0F, 0.0F, false);
		cube_r11.setTextureOffset(30, 53).addBox(5.4021F, -9.8F, -12.1612F, 4.0F, 2.0F, 4.0F, 0.0F, false);
		cube_r11.setTextureOffset(22, 20).addBox(5.4021F, -13.8F, -12.1612F, 4.0F, 4.0F, 4.0F, 0.0F, false);

		bone10 = new ModelRenderer(this);
		bone10.setRotationPoint(0.0F, 0.0F, 0.0F);
		body.addChild(bone10);
		setRotationAngle(bone10, 0.1309F, 0.3054F, 0.0F);
		

		cube_r12 = new ModelRenderer(this);
		cube_r12.setRotationPoint(0.0F, 0.0F, 0.0F);
		bone10.addChild(cube_r12);
		setRotationAngle(cube_r12, -0.2618F, 0.0F, 0.0F);
		cube_r12.setTextureOffset(22, 19).addBox(10.7F, -23.8F, -6.2F, 4.0F, 6.0F, 4.0F, 0.0F, false);
		cube_r12.setTextureOffset(23, 21).addBox(10.7F, -17.8F, -6.2F, 4.0F, 3.0F, 4.0F, 0.0F, false);
		cube_r12.setTextureOffset(38, 24).addBox(10.7F, -14.8F, -6.2F, 4.0F, 4.0F, 4.0F, 0.0F, false);

		cube_r13 = new ModelRenderer(this);
		cube_r13.setRotationPoint(-0.3F, -15.9F, 3.8F);
		bone10.addChild(cube_r13);
		setRotationAngle(cube_r13, -0.4422F, 0.158F, -0.0744F);
		cube_r13.setTextureOffset(30, 57).addBox(-0.0604F, 0.7072F, -6.648F, 4.0F, 3.0F, 4.0F, 0.0F, false);
		cube_r13.setTextureOffset(30, 56).addBox(-0.0604F, 0.7072F, -10.648F, 4.0F, 3.0F, 4.0F, 0.0F, false);

		cube_r14 = new ModelRenderer(this);
		cube_r14.setRotationPoint(-0.3F, 1.1F, 3.8F);
		bone10.addChild(cube_r14);
		setRotationAngle(cube_r14, -0.4367F, 0.0395F, -0.0184F);
		cube_r14.setTextureOffset(30, 56).addBox(-0.4F, -15.7F, -13.9F, 4.0F, 1.0F, 4.0F, 0.0F, false);
		cube_r14.setTextureOffset(22, 19).addBox(-0.4F, -21.7F, -13.9F, 4.0F, 6.0F, 4.0F, 0.0F, false);

		tail = new ModelRenderer(this);
		tail.setRotationPoint(6.0F, -20.5F, -0.8F);
		body.addChild(tail);
		

		bone = new ModelRenderer(this);
		bone.setRotationPoint(0.0F, 0.0F, 0.0F);
		tail.addChild(bone);
		setRotationAngle(bone, -0.1744F, 0.0062F, -0.0014F);
		

		cube_r15 = new ModelRenderer(this);
		cube_r15.setRotationPoint(0.0F, 0.0F, 0.0F);
		bone.addChild(cube_r15);
		setRotationAngle(cube_r15, 0.0F, 0.0F, -3.1416F);
		cube_r15.setTextureOffset(159, 0).addBox(-4.0F, -4.0F, -1.0F, 4.0F, 6.0F, 5.0F, 0.0F, false);

		cube_r16 = new ModelRenderer(this);
		cube_r16.setRotationPoint(0.0F, -6.0F, 0.0F);
		bone.addChild(cube_r16);
		setRotationAngle(cube_r16, 0.0F, 0.0F, -3.1416F);
		cube_r16.setTextureOffset(159, 0).addBox(-4.0F, -4.0F, -1.0F, 4.0F, 9.0F, 5.0F, 0.0F, false);

		cube_r17 = new ModelRenderer(this);
		cube_r17.setRotationPoint(0.0F, 6.0F, 0.0F);
		bone.addChild(cube_r17);
		setRotationAngle(cube_r17, 0.0F, 0.0F, -3.1416F);
		cube_r17.setTextureOffset(159, 0).addBox(-4.0F, -4.0F, -1.0F, 4.0F, 6.0F, 5.0F, 0.0F, false);

		bone2 = new ModelRenderer(this);
		bone2.setRotationPoint(-3.0F, 1.1F, 0.0F);
		tail.addChild(bone2);
		setRotationAngle(bone2, -0.1729F, -0.024F, -0.1733F);
		

		cube_r18 = new ModelRenderer(this);
		cube_r18.setRotationPoint(0.0F, 0.0F, 0.0F);
		bone2.addChild(cube_r18);
		setRotationAngle(cube_r18, 0.0F, 0.0F, -3.1416F);
		cube_r18.setTextureOffset(159, 0).addBox(-4.0F, -4.0F, -1.0F, 4.0F, 6.0F, 5.0F, 0.0F, false);

		cube_r19 = new ModelRenderer(this);
		cube_r19.setRotationPoint(0.0F, -6.0F, 0.0F);
		bone2.addChild(cube_r19);
		setRotationAngle(cube_r19, 0.0F, 0.0F, -3.1416F);
		cube_r19.setTextureOffset(159, 0).addBox(-4.0F, -4.0F, -1.0F, 4.0F, 9.0F, 5.0F, 0.0F, false);

		cube_r20 = new ModelRenderer(this);
		cube_r20.setRotationPoint(0.0F, 6.0F, 0.0F);
		bone2.addChild(cube_r20);
		setRotationAngle(cube_r20, 0.0F, 0.0F, -3.1416F);
		cube_r20.setTextureOffset(159, 0).addBox(-4.0F, -4.0F, -1.0F, 4.0F, 6.0F, 5.0F, 0.0F, false);

		bone3 = new ModelRenderer(this);
		bone3.setRotationPoint(2.9F, 0.0F, 0.0F);
		tail.addChild(bone3);
		setRotationAngle(bone3, -0.169F, 0.0436F, 0.2136F);
		

		cube_r21 = new ModelRenderer(this);
		cube_r21.setRotationPoint(0.0F, 0.0F, 0.0F);
		bone3.addChild(cube_r21);
		setRotationAngle(cube_r21, 0.0F, 0.0F, -3.1416F);
		cube_r21.setTextureOffset(159, 0).addBox(-4.0F, -4.0F, -1.0F, 4.0F, 6.0F, 5.0F, 0.0F, false);

		cube_r22 = new ModelRenderer(this);
		cube_r22.setRotationPoint(0.0F, -6.0F, 0.0F);
		bone3.addChild(cube_r22);
		setRotationAngle(cube_r22, 0.0F, 0.0F, -3.1416F);
		cube_r22.setTextureOffset(159, 0).addBox(-4.0F, -4.0F, -1.0F, 4.0F, 9.0F, 5.0F, 0.0F, false);

		cube_r23 = new ModelRenderer(this);
		cube_r23.setRotationPoint(0.0F, 6.0F, 0.0F);
		bone3.addChild(cube_r23);
		setRotationAngle(cube_r23, 0.0F, 0.0F, -3.1416F);
		cube_r23.setTextureOffset(159, 0).addBox(-4.0F, -4.0F, -1.0F, 4.0F, 6.0F, 5.0F, 0.0F, false);

		bone4 = new ModelRenderer(this);
		bone4.setRotationPoint(5.6F, 1.2F, 0.0F);
		tail.addChild(bone4);
		setRotationAngle(bone4, -0.159F, 0.0722F, 0.3861F);
		

		cube_r24 = new ModelRenderer(this);
		cube_r24.setRotationPoint(0.0F, 0.0F, 0.0F);
		bone4.addChild(cube_r24);
		setRotationAngle(cube_r24, 0.0F, 0.0F, -3.1416F);
		cube_r24.setTextureOffset(159, 0).addBox(-4.0F, -4.0F, -1.0F, 4.0F, 6.0F, 5.0F, 0.0F, false);

		cube_r25 = new ModelRenderer(this);
		cube_r25.setRotationPoint(0.0F, -6.0F, 0.0F);
		bone4.addChild(cube_r25);
		setRotationAngle(cube_r25, 0.0F, 0.0F, -3.1416F);
		cube_r25.setTextureOffset(159, 0).addBox(-4.0F, -4.0F, -1.0F, 4.0F, 9.0F, 5.0F, 0.0F, false);

		cube_r26 = new ModelRenderer(this);
		cube_r26.setRotationPoint(0.0F, 6.0F, 0.0F);
		bone4.addChild(cube_r26);
		setRotationAngle(cube_r26, 0.0F, 0.0F, -3.1416F);
		cube_r26.setTextureOffset(159, 0).addBox(-4.0F, -4.0F, -1.0F, 4.0F, 6.0F, 5.0F, 0.0F, false);

		bone5 = new ModelRenderer(this);
		bone5.setRotationPoint(-6.3F, 3.1F, 0.0F);
		tail.addChild(bone5);
		setRotationAngle(bone5, -0.1683F, -0.0463F, -0.3024F);
		

		cube_r27 = new ModelRenderer(this);
		cube_r27.setRotationPoint(0.0F, 0.0F, 0.0F);
		bone5.addChild(cube_r27);
		setRotationAngle(cube_r27, 0.0F, 0.0F, -3.1416F);
		cube_r27.setTextureOffset(159, 0).addBox(-4.0F, -4.0F, -1.0F, 4.0F, 6.0F, 5.0F, 0.0F, false);

		cube_r28 = new ModelRenderer(this);
		cube_r28.setRotationPoint(0.0F, -6.0F, 0.0F);
		bone5.addChild(cube_r28);
		setRotationAngle(cube_r28, 0.0F, 0.0F, -3.1416F);
		cube_r28.setTextureOffset(159, 0).addBox(-4.0F, -4.0F, -1.0F, 4.0F, 9.0F, 5.0F, 0.0F, false);

		cube_r29 = new ModelRenderer(this);
		cube_r29.setRotationPoint(0.0F, 6.0F, 0.0F);
		bone5.addChild(cube_r29);
		setRotationAngle(cube_r29, 0.0F, 0.0F, -3.1416F);
		cube_r29.setTextureOffset(159, 0).addBox(-4.0F, -4.0F, -1.0F, 4.0F, 6.0F, 5.0F, 0.0F, false);

		bone6 = new ModelRenderer(this);
		bone6.setRotationPoint(-6.8F, 7.7F, 1.0F);
		tail.addChild(bone6);
		setRotationAngle(bone6, -0.5416F, 0.3817F, -0.8598F);
		

		cube_r30 = new ModelRenderer(this);
		cube_r30.setRotationPoint(0.0F, 0.0F, 0.0F);
		bone6.addChild(cube_r30);
		setRotationAngle(cube_r30, 0.0F, 0.0F, -3.1416F);
		cube_r30.setTextureOffset(159, 0).addBox(-4.0F, -4.0F, -1.0F, 4.0F, 6.0F, 5.0F, 0.0F, false);

		cube_r31 = new ModelRenderer(this);
		cube_r31.setRotationPoint(0.0F, -6.0F, 0.0F);
		bone6.addChild(cube_r31);
		setRotationAngle(cube_r31, 0.0F, 0.0F, -3.1416F);
		cube_r31.setTextureOffset(159, 0).addBox(-4.0F, -4.0F, -1.0F, 4.0F, 9.0F, 5.0F, 0.0F, false);

		cube_r32 = new ModelRenderer(this);
		cube_r32.setRotationPoint(0.0F, 6.0F, 0.0F);
		bone6.addChild(cube_r32);
		setRotationAngle(cube_r32, 0.0F, 0.0F, -3.1416F);
		cube_r32.setTextureOffset(159, 0).addBox(-4.0F, -4.0F, -1.0F, 4.0F, 6.0F, 5.0F, 0.0F, false);

		bone7 = new ModelRenderer(this);
		bone7.setRotationPoint(-6.8F, 8.6F, 1.0F);
		tail.addChild(bone7);
		setRotationAngle(bone7, -0.6339F, 0.1618F, -1.1985F);
		

		cube_r33 = new ModelRenderer(this);
		cube_r33.setRotationPoint(0.0F, 0.0F, 0.0F);
		bone7.addChild(cube_r33);
		setRotationAngle(cube_r33, 0.0F, 0.0F, -3.1416F);
		cube_r33.setTextureOffset(159, 0).addBox(-4.0F, -4.0F, -1.0F, 4.0F, 6.0F, 5.0F, 0.0F, false);

		cube_r34 = new ModelRenderer(this);
		cube_r34.setRotationPoint(0.0F, -6.0F, 0.0F);
		bone7.addChild(cube_r34);
		setRotationAngle(cube_r34, 0.0F, 0.0F, -3.1416F);
		cube_r34.setTextureOffset(159, 0).addBox(-4.0F, -4.0F, -1.0F, 4.0F, 9.0F, 5.0F, 0.0F, false);

		cube_r35 = new ModelRenderer(this);
		cube_r35.setRotationPoint(0.0F, 6.0F, 0.0F);
		bone7.addChild(cube_r35);
		setRotationAngle(cube_r35, 0.0F, 0.0F, -3.1416F);
		cube_r35.setTextureOffset(159, 0).addBox(-4.0F, -4.0F, -1.0F, 4.0F, 6.0F, 5.0F, 0.0F, false);

		bone8 = new ModelRenderer(this);
		bone8.setRotationPoint(7.8F, 4.9F, -0.6F);
		tail.addChild(bone8);
		setRotationAngle(bone8, -0.3623F, -0.0977F, 0.6999F);
		

		cube_r36 = new ModelRenderer(this);
		cube_r36.setRotationPoint(0.0F, 0.0F, 0.0F);
		bone8.addChild(cube_r36);
		setRotationAngle(cube_r36, 0.0F, 0.0F, -3.1416F);
		cube_r36.setTextureOffset(159, 0).addBox(-4.0F, -4.0F, -1.0F, 4.0F, 6.0F, 5.0F, 0.0F, false);

		cube_r37 = new ModelRenderer(this);
		cube_r37.setRotationPoint(0.0F, -6.0F, 0.0F);
		bone8.addChild(cube_r37);
		setRotationAngle(cube_r37, 0.0F, 0.0F, -3.1416F);
		cube_r37.setTextureOffset(159, 0).addBox(-4.0F, -4.0F, -1.0F, 4.0F, 9.0F, 5.0F, 0.0F, false);

		cube_r38 = new ModelRenderer(this);
		cube_r38.setRotationPoint(0.0F, 6.0F, 0.0F);
		bone8.addChild(cube_r38);
		setRotationAngle(cube_r38, 0.0F, 0.0F, -3.1416F);
		cube_r38.setTextureOffset(159, 0).addBox(-4.0F, -4.0F, -1.0F, 4.0F, 6.0F, 5.0F, 0.0F, false);

		bone9 = new ModelRenderer(this);
		bone9.setRotationPoint(7.8F, 5.6F, -0.9F);
		tail.addChild(bone9);
		setRotationAngle(bone9, -0.4964F, -0.1016F, 1.2616F);
		

		cube_r39 = new ModelRenderer(this);
		cube_r39.setRotationPoint(0.0F, 0.0F, 0.0F);
		bone9.addChild(cube_r39);
		setRotationAngle(cube_r39, 0.0F, 0.0F, -3.1416F);
		cube_r39.setTextureOffset(159, 0).addBox(-4.0F, -4.0F, -1.0F, 4.0F, 6.0F, 5.0F, 0.0F, false);

		cube_r40 = new ModelRenderer(this);
		cube_r40.setRotationPoint(0.0F, -6.0F, 0.0F);
		bone9.addChild(cube_r40);
		setRotationAngle(cube_r40, 0.0F, 0.0F, -3.1416F);
		cube_r40.setTextureOffset(159, 0).addBox(-4.0F, -4.0F, -1.0F, 4.0F, 9.0F, 5.0F, 0.0F, false);

		cube_r41 = new ModelRenderer(this);
		cube_r41.setRotationPoint(0.0F, 6.0F, 0.0F);
		bone9.addChild(cube_r41);
		setRotationAngle(cube_r41, 0.0F, 0.0F, -3.1416F);
		cube_r41.setTextureOffset(159, 0).addBox(-4.0F, -4.0F, -1.0F, 4.0F, 6.0F, 5.0F, 0.0F, false);
	}

	@Override
	public void setRotationAngles(Entity entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch){
		//previously the render function, render code was moved to a method below
	}

	@Override
	public void render(MatrixStack matrixStack, IVertexBuilder buffer, int packedLight, int packedOverlay, float red, float green, float blue, float alpha){
		support.render(matrixStack, buffer, packedLight, packedOverlay);
		Main.render(matrixStack, buffer, packedLight, packedOverlay);
	}

	public void setRotationAngle(ModelRenderer modelRenderer, float x, float y, float z) {
		modelRenderer.rotateAngleX = x;
		modelRenderer.rotateAngleY = y;
		modelRenderer.rotateAngleZ = z;
	}
}