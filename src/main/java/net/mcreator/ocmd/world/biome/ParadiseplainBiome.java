...
public class ParadiseplainBiome extends OcmModElements.ModElement {
	public static Biome biome;

	public ParadiseplainBiome(OcmModElements instance) {
		super(instance, 1539);
	...
	private static class BiomeRegisterHandler {
		...
		public void registerBiomes(RegistryEvent.Register<Biome> event) {
			if (biome == null) {
				BiomeAmbience effects = new BiomeAmbience.Builder().setFogColor(-15369788).setWaterColor(-14264194).setWaterFogColor(-14264194)
						.withSkyColor(-15369788).withFoliageColor(-13140442).withGrassColor(-13140442).build();
				...
				biome = new Biome.Builder().precipitation(Biome.RainType.RAIN).category(Biome.Category.THEEND).depth(-1f).scale(2f).temperature(0.5f)
						.downfall(0.5f).setEffects(effects).withMobSpawnSettings(mobSpawnInfo.copy())
						.withGenerationSettings(biomeGenerationSettings.build()).build();
				event.getRegistry().register(biome.setRegistryName("ocm:paradiseplain"));
			}
		}
	}
