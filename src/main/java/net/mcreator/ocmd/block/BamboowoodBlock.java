...
public class BamboowoodBlock extends OcmModElements.ModElement {
	@ObjectHolder("ocm:bamboowood")
	public static final Block block = null;

	public BamboowoodBlock(OcmModElements instance) {
		super(instance, 1154);
	...
	private static class FeatureRegisterHandler {
		...
		public void registerFeature(RegistryEvent.Register<Feature<?>> event) {
				...
				public boolean generate(ISeedReader world, ChunkGenerator generator, Random random, BlockPos pos, BlockClusterFeatureConfig config) {
					...
					for (int j = 0; j < 40; ++j) {
						...
						if (world.isAirBlock(blockpos)) {
							...
							int k = 1 + random.nextInt(random.nextInt(12) + 1);
							k = Math.min(12, k);
							for (int l = 0; l < k; ++l) {
			...
			event.getRegistry().register(feature.setRegistryName("bamboowood"));
			Registry.register(WorldGenRegistries.CONFIGURED_FEATURE, new ResourceLocation("ocm:bamboowood"), configuredFeature);
		}
	}
	...
	public void addFeatureToBiomes(BiomeLoadingEvent event) {
		...
		if (new ResourceLocation("ocm:japanplain").equals(event.getName()))
			biomeCriteria = true;
		if (new ResourceLocation("ocm:japanbambooforest").equals(event.getName()))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
